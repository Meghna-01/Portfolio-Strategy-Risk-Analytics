...
LGD = st.sidebar.slider(
    ...
    value=st.session_state.lgd_value, step=5) / 100
st.session_state.lgd_value = int(LGD * 100)
st.sidebar.caption(
    f"Loss Given Default = {int(LGD*100)}% "
    f"(percentage of loan value not recovered after default)")
st.sidebar.markdown("---")
...
st.markdown(SCROLL_TOP, unsafe_allow_html=True)
    ...
    with col3:
        ...
        chart_caption(f"Net Yield = Avg Interest Rate − (Default Rate × LGD). Currently using {int(LGD*100)}% LGD. Cyan = compensated. Red = loss-making.")
...
st.markdown(SCROLL_TOP, unsafe_allow_html=True)
    ...
    for r in stress_rows:
        ...
        el_body += (
            ...
            f'<td style="padding:12px 16px;text-align:right;color:#ccc;font-size:14px;">{int(LGD*100)}%</td>'
    ...
    el_body += (
        ...
        f'<td style="padding:12px 16px;text-align:right;color:#fff;font-weight:700;font-size:14px;">${total_exp_mn} Mn</td>'
        f'<td style="padding:12px 16px;text-align:right;color:#aaa;font-size:14px;">—</td>'
        f'<td style="padding:12px 16px;text-align:right;color:#aaa;font-size:14px;">{int(LGD*100)}%</td>'
    ...
    base_port_el = round((prime_data["loan_amnt"].sum()*prime_pd*LGD +
        near_data["loan_amnt"].sum()*near_pd*LGD + sub_exp_raw*sub_pd*LGD +
        high_exp_raw*high_pd*LGD)/1_000_000, 2)

    new_port_el = round((prime_exp_new*prime_pd*LGD + near_exp_new*near_pd*LGD +
        sub_exp_new*sub_pd*LGD + high_exp_new*high_pd*LGD)/1_000_000, 2)
...
st.markdown(SCROLL_TOP, unsafe_allow_html=True)
    ...
    kpi_grid([
        ...
        ("LGD Assumption",           f"{int(LGD*100)}%",    "Adjust on sidebar slider"),
    ])
    ...
    findings = [
        ...
        f"fully compensate for default losses in lower credit grades at {int(LGD*100)}% LGD.",
    ...
    for f in findings:
        fhtml += f'<p style="color:#cccccc;font-size:15px !important;margin-bottom:14px;line-height:1.75;">→ {f}</p>'
    fhtml += '</div>'
