...
st.markdown("""
    <style>
    /* Clean up native Streamlit spacing */
...
# Build anchor data used across multiple pages
anchor_rows = []
for seg in ORDER:
    s = df[df["segment"] == seg]
    n = len(s)
    ...
    anchor_rows.append({
        "seg": seg, "n": n, "exp": exp, "exp_pct": exp_pct,
        "dr": dr, "loss_pct": loss_pct, "base_pd": base_pd
    })
...
st.sidebar.caption(f"Loss Given Default = {int(LGD*100)}% (percentage of loan value not recovered after default). Adjusting this updates Net Yield and Stress Testing globally.")
st.sidebar.markdown("---")
st.sidebar.caption("Built by Meghna")

# Auto-scroll script for page changes
SCROLL_TOP = """
    <script>
...
def chart_header(text):
    st.markdown(f"<h3 style='text-align:center; color:white; margin-bottom:4px; font-size:18px;'>{text}</h3>", unsafe_allow_html=True)

def chart_caption(text):
    st.markdown(f"<p style='text-align:center; color:#aaaaaa; font-size:13px; margin-top:0; margin-bottom:12px;'>{text}</p>", unsafe_allow_html=True)

def col_divider():
    st.markdown("<div style='border-left:1px solid #2d3447; height:100%; min-height:350px;'></div>", unsafe_allow_html=True)
...
def metric_row(labels, values):
    ...
    for label, value in zip(labels, values):
        ...
        border-radius:10px; flex:1; min-width:0; text-align:center;">
            <p style="color:#aaaaaa; font-size:12px; text-transform:uppercase; font-weight:600; margin:0 0 8px 0; white-space:nowrap;">{label}</p>
            <p style="color:#00B4D8; font-size:26px; font-weight:bold; margin:0; white-space:nowrap;">{value}</p>
        </div>"""
    st.markdown(f'<div style="display:flex; gap:14px; margin-bottom:24px;">{cards}</div>', unsafe_allow_html=True)
...
if page == "📊 Executive Overview":
    ...
    total_exposure_mn = total_exposure_all / 1_000_000
    default_rate = round(df["loan_status"].mean() * 100, 1)
    avg_risk_score = round(df["risk_score"].mean(), 1)
    high_risk_pct = round(len(df[df["segment"].isin(["SUBPRIME", "HIGH-RISK"])]) / total_loans_all * 100, 1)

    metric_row(
        ["Total Loans", "Total Exposure", "Default Rate", "Avg Risk Score", "At-Risk Exposure"],
        [f"{total_loans_all:,}", f"${total_exposure_mn:,.1f} Mn", f"{default_rate}%", f"{avg_risk_score}", f"{high_risk_pct}%"]
    )
    ...
    # Re-using the organic generated histogram look
    ...
    fig_dist.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color="white",
    ...
    with col1:
        ...
        fig1.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color="white", showlegend=True, margin=dict(t=10, b=10, l=10, r=10), height=360)
...
# ==========================================
    ...
    chart_caption(
        "689 borrowers rated Grade B landed in SUBPRIME. In Grade C, 777 landed in SUBPRIME and 209 in HIGH-RISK. "
    ...
    fig_heat = px.imshow(cross, color_continuous_scale=["#0E1117", "#00B4D8"], text_auto=True, aspect="auto", labels=dict(x="Risk Segment", y="Loan Grade", color="# Borrowers"))
    fig_heat.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color="white", margin=dict(t=20, b=10, l=10, r=10), height=400)
    ...
    with col1:
        ...
        for i, row in grade_exposure.iterrows():
            fig1.add_trace(go.Bar(
                ...
                textposition="outside", textfont=dict(size=14, color="white"),
                marker_color=grade_colors[i % len(grade_colors)], width=0.6, showlegend=False, cliponaxis=False))
        fig1.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color="white", yaxis_title="Exposure ($ Mn)", yaxis=dict(gridcolor="#1f2630", range=[0, grade_exposure["Exposure ($ Mn)"].max() * 1.25]), margin=dict(t=30, b=10, l=10, r=10), height=380)
        st.plotly_chart(fig1, use_container_width=True)
    ...
    with col2:
        ...
        for i, row in grade_default.iterrows():
            ...
            fig2.add_trace(go.Bar(
                ...
                text=[str(row["Default Rate"]) + "%"],
                textposition="outside", textfont=dict(size=14, color="white"),
                marker_color=f"rgb({r},{g},{b})", width=0.6, showlegend=False, cliponaxis=False))
        fig2.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color="white", yaxis=dict(gridcolor="#1f2630", range=[0, grade_default["Default Rate"].max() * 1.25]), margin=dict(t=30, b=10, l=10, r=10), height=380)
        st.plotly_chart(fig2, use_container_width=True)

    st.markdown("---")

    col3, divider_col2, col4 = st.columns([1, 0.03, 1])
    with divider_col2: col_divider()
    
    with col3:
        chart_header("Risk-Adjusted Net Yield by Grade")
        chart_caption(f"Currently using {int(LGD*100)}% LGD from Sidebar. Cyan = compensated. Red = loss-making.")
        ...
        for _, row in grade_rar.iterrows():
            fig3.add_trace(go.Bar(
                ...
                text=[str(row["Net_Yield"]) + "%"], textposition="outside", textfont=dict(size=14, color="white"),
                marker_color="#00B4D8" if row["Net_Yield"] >= 0 else "#E63946", width=0.6, showlegend=False, cliponaxis=False))
        fig3.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color="white", yaxis_title="Net Yield (%)", yaxis=dict(gridcolor="#1f2630", range=[min_yield * 1.2, max_yield * 1.5]), margin=dict(t=30, b=10, l=10, r=10), height=400)
    ...
    with col4:
        ...
        fig4 = px.scatter(sample, x="risk_score", y="loan_int_rate", color="segment", color_discrete_sequence=PALETTE, category_orders={"segment": ORDER}, opacity=0.6, labels={"risk_score": "Risk Score", "loan_int_rate": "Interest Rate (%)", "segment": "Segment"})
        fig4.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color="white", xaxis=dict(gridcolor="#1f2630"), yaxis=dict(gridcolor="#1f2630"), margin=dict(t=20, b=10, l=10, r=10), height=400)
...
# ==========================================
    ...
    # Calculate targets using anchor_rows
    alloc_multiplier = {"Grow": 1.1, "Maintain": 1.0, "Tighten": 0.5, "Exit": 0.0}
    
    for r in anchor_rows:
        ep = r["exp_pct"]; lp = r["loss_pct"]
        if lp > ep * 2: r["rec"] = "Exit"
        elif lp > ep: r["rec"] = "Tighten"
        elif lp < ep * 0.8: r["rec"] = "Grow"
        else: r["rec"] = "Maintain"

    raw_targets = {r["seg"]: r["exp_pct"] * alloc_multiplier[r["rec"]] for r in anchor_rows}
    ...
    with col1:
        ...
        exp_loss_data = pd.DataFrame({
            ...
            "Value": ([r["exp_pct"] for r in anchor_rows] + [r["loss_pct"] for r in anchor_rows])})
        exp_loss_data["Segment"] = pd.Categorical(exp_loss_data["Segment"], categories=ORDER, ordered=True)
        fig1 = px.bar(exp_loss_data, x="Segment", y="Value", color="Metric", barmode="group", text="Value", color_discrete_map={"Exposure %": "#00B4D8", "Loss Contribution %": "#E63946"}, category_orders={"Segment": ORDER})
        fig1.update_traces(texttemplate="%{text}%", textposition="outside", textfont=dict(size=14), cliponaxis=False)
        fig1.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color="white", yaxis_title="Percentage (%)", yaxis=dict(gridcolor="#1f2630", range=[0, exp_loss_data["Value"].max() * 1.3]), bargap=0.2, bargroupgap=0.05, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), margin=dict(t=40, b=40, l=10, r=10), height=420)
        st.plotly_chart(fig1, use_container_width=True)
        
    with col2:
        chart_header("Recommended Portfolio Reallocation")
        max_alloc = max(max(r["exp_pct"] for r in anchor_rows), max(target_alloc[seg] for seg in ORDER))
        alloc_df = pd.DataFrame({"Segment": ORDER, "Current %": [r["exp_pct"] for r in anchor_rows], "Target %": [target_alloc[seg] for seg in ORDER]})
        ...
        fig2 = px.bar(alloc_melted, y="Segment", x="Value", color="Type", barmode="group", text="Value", orientation="h", color_discrete_map={"Current %": "#FFB703", "Target %": "#00B4D8"}, category_orders={"Segment": ORDER})
        fig2.update_traces(texttemplate="%{x}%", textposition="outside", textfont=dict(size=14), cliponaxis=False)
        fig2.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color="white", xaxis_title="Allocation (%)", xaxis=dict(gridcolor="#1f2630", range=[0, max_alloc * 1.3]), bargap=0.2, bargroupgap=0.1, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), margin=dict(t=40, b=40, l=10, r=80), height=420)
        st.plotly_chart(fig2, use_container_width=True)
...
# ==========================================
    ...
    st.caption(f"Currently using {int(LGD*100)}% LGD assumption from the sidebar. EL = Exposure × Default Rate × LGD.")
    ...
    multiplier_map = {"Base Case (No Stress)": 1.00, "Mild Recession (+25% PD)": 1.25,
        "Severe Recession (+50% PD)": 1.50, "Extreme Stress (+75% PD)": 1.75}
    pd_mult = multiplier_map[scenario]
    ...
    sc2.metric("Stressed Expected Loss", f"${stressed_total_el} Mn", delta=f"+${el_increase} Mn", delta_color="inverse")
    sc3.metric("Increase in Loss", f"{el_increase_pct}%", delta=f"{el_increase_pct}%", delta_color="inverse")

    st.markdown("")

    s_header = ('<div style="overflow-x:auto; margin-bottom:24px;"><table style="width:100%; border-collapse:collapse; font-size:15px;"><thead><tr style="background:#1a1f2e; color:#aaaaaa; font-size:13px;"><th style="padding:14px 16px; text-align:left; white-space:nowrap; border-bottom:1px solid #2d3447;">Segment</th><th style="padding:14px 16px; text-align:right; white-space:nowrap; border-bottom:1px solid #2d3447;">Base Default Rate</th><th style="padding:14px 16px; text-align:right; white-space:nowrap; border-bottom:1px solid #2d3447;">Stressed Default Rate</th><th style="padding:14px 16px; text-align:right; white-space:nowrap; border-bottom:1px solid #2d3447;">Base Exp. Loss</th><th style="padding:14px 16px; text-align:right; white-space:nowrap; border-bottom:1px solid #2d3447;">Stressed Exp. Loss</th><th style="padding:14px 16px; text-align:right; white-space:nowrap; border-bottom:1px solid #2d3447;">Additional Loss</th></tr></thead><tbody>')
    s_body = ""
    for sr in stress_rows:
        change_color = "#E63946" if sr["el_change"] > 0 else "#00B4D8"
        s_body += ('<tr style="background:#0e1117; border-bottom:1px solid #1a1f2e;"><td style="padding:13px 16px; color:white; font-weight:600; white-space:nowrap;">' + sr["seg"] + '</td><td style="padding:13px 16px; text-align:right; color:#cccccc; white-space:nowrap;">' + f"{sr['base_dr']}%" + '</td><td style="padding:13px 16px; text-align:right; color:#FFB703; white-space:nowrap;">' + f"{sr['stressed_dr']}%" + '</td><td style="padding:13px 16px; text-align:right; color:#cccccc; white-space:nowrap;">$' + f"{sr['base_el']} Mn" + '</td><td style="padding:13px 16px; text-align:right; color:#FFB703; white-space:nowrap;">$' + f"{sr['stressed_el']} Mn" + '</td><td style="padding:13px 16px; text-align:right; white-space:nowrap; color:' + change_color + '; font-weight:700;">+$' + f"{sr['el_change']} Mn" + '</td></tr>')
    ...
    with col1:
        ...
        el_compare = pd.DataFrame({"Segment": ORDER * 2, "Scenario": ["Base Case"] * 4 + [scenario] * 4, "Expected Loss ($ Mn)": ([r["base_el"] for r in stress_rows] + [r["stressed_el"] for r in stress_rows])})
        el_compare["Segment"] = pd.Categorical(el_compare["Segment"], categories=ORDER, ordered=True)
        fig1 = px.bar(el_compare, x="Segment", y="Expected Loss ($ Mn)", color="Scenario", barmode="group", text="Expected Loss ($ Mn)", color_discrete_map={"Base Case": "#00B4D8", scenario: "#E63946"}, category_orders={"Segment": ORDER})
        fig1.update_traces(texttemplate="$%{text} Mn", textposition="outside", textfont=dict(size=13), cliponaxis=False)
        fig1.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color="white", yaxis=dict(gridcolor="#1f2630", range=[0, el_compare["Expected Loss ($ Mn)"].max() * 1.3]), bargap=0.2, bargroupgap=0.05, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), margin=dict(t=40, b=10, l=10, r=10), height=380)
        st.plotly_chart(fig1, use_container_width=True)
        
    with col2:
        chart_header("Default Rate: Base vs Stressed")
        dr_compare = pd.DataFrame({"Segment": ORDER * 2, "Scenario": ["Base Case"] * 4 + [scenario] * 4, "Default Rate (%)": ([r["base_dr"] for r in stress_rows] + [r["stressed_dr"] for r in stress_rows])})
        dr_compare["Segment"] = pd.Categorical(dr_compare["Segment"], categories=ORDER, ordered=True)
        fig2 = px.bar(dr_compare, x="Segment", y="Default Rate (%)", color="Scenario", barmode="group", text="Default Rate (%)", color_discrete_map={"Base Case": "#00B4D8", scenario: "#E63946"}, category_orders={"Segment": ORDER})
        fig2.update_traces(texttemplate="%{text}%", textposition="outside", textfont=dict(size=13), cliponaxis=False)
        fig2.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color="white", yaxis=dict(gridcolor="#1f2630", range=[0, dr_compare["Default Rate (%)"].max() * 1.3]), bargap=0.2, bargroupgap=0.05, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), margin=dict(t=40, b=10, l=10, r=10), height=380)
        st.plotly_chart(fig2, use_container_width=True)

    st.markdown("---")

    st.subheader("Section 2 — Future Capital Reallocation Simulator")
    st.caption(
        "Simulates future lending decisions — not movement of existing loans. "
        "Redirects future originations from SUBPRIME and HIGH-RISK toward PRIME "
        "while keeping total portfolio size constant. "
        "The split is weighted proportionally based on where the risky exposure sits.")
    st.markdown("")

    shift_pct = st.slider(
        "Future Capital Reallocation (% of total portfolio redirected from SUBPRIME + HIGH-RISK → PRIME)",
    ...
    base_port_el = round((prime_data["loan_amnt"].sum() * prime_pd * LGD + near_data["loan_amnt"].sum() * near_pd * LGD + sub_exp_raw * sub_pd * LGD + high_exp_raw * high_pd * LGD) / 1_000_000, 2)
    new_port_el = round((prime_exp_new * prime_pd * LGD + near_exp_new * near_pd * LGD + sub_exp_new * sub_pd * LGD + high_exp_new * high_pd * LGD) / 1_000_000, 2)
    base_port_dr = round(df["loan_status"].mean() * 100, 2)
    ...
    r2.metric("Simulated Default Rate", f"{new_port_dr}%", delta=f"-{dr_improvement}%", delta_color="inverse")
    r3.metric("Base Expected Loss", f"${base_port_el} Mn")
    r4.metric("Simulated Expected Loss", f"${new_port_el} Mn", delta=f"-${el_saved} Mn", delta_color="inverse")
    ...
    fig_sim.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color="white", yaxis_title="Exposure ($ Mn)", yaxis=dict(gridcolor="#1f2630", range=[0, realloc_data["Exposure ($ Mn)"].max() * 1.3]), bargap=0.2, bargroupgap=0.05, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), margin=dict(t=40, b=10, l=10, r=10), height=400)
...
# ==========================================
    ...
    actions = [
        ("PRIME", rec_map["PRIME"][0], rec_map["PRIME"][1],
         ...
         f"{seg_stats['PRIME']['dr']}% default rate. Loss contribution well below exposure share."),
        ("NEAR-PRIME", rec_map["NEAR-PRIME"][0], rec_map["NEAR-PRIME"][1],
         "Maintain with stricter underwriting on LTI > 25%",
         f"{seg_stats['NEAR-PRIME']['dr']}% default rate. Loss {seg_stats['NEAR-PRIME']['loss_pct']}% vs exposure {seg_stats['NEAR-PRIME']['exp_pct']}%."),
    ...
    a_header = ('<div style="overflow-x:auto;"><table style="width:100%; border-collapse:collapse; font-size:15px;"><thead><tr style="background:#1a1f2e; color:#aaaaaa; font-size:13px;"><th style="padding:14px 16px; text-align:left; white-space:nowrap; border-bottom:1px solid #2d3447;">Segment</th><th style="padding:14px 16px; text-align:center; white-space:nowrap; border-bottom:1px solid #2d3447;">Action</th><th style="padding:14px 16px; text-align:left; white-space:nowrap; border-bottom:1px solid #2d3447;">What to Do</th><th style="padding:14px 16px; text-align:left; border-bottom:1px solid #2d3447;">Why</th></tr></thead><tbody>')
    a_body = ""
    ...
    for seg, rec, color, what, why in actions:
        a_body += ('<tr style="background:#0e1117; border-bottom:1px solid #1a1f2e;"><td style="padding:13px 16px; color:white; font-weight:600; white-space:nowrap;">' + seg + '</td><td style="padding:13px 16px; text-align:center; white-space:nowrap;"><span style="background-color:' + color + '22; color:' + color + '; border:1px solid ' + color + '; ' + badge_style + '">' + rec + '</span></td><td style="padding:13px 16px; color:#cccccc; white-space:nowrap;">' + what + '</td><td style="padding:13px 16px; color:#888888; font-size:13px;">' + why + '</td></tr>')
    st.markdown(a_header + a_body + "</tbody></table></div>", unsafe_allow_html=True)

    st.markdown("---")
    chart_header("Current vs Target Portfolio Allocation")

    final_alloc = pd.DataFrame({"Segment": ORDER, "Current %": [seg_stats[seg]["exp_pct"] for seg in ORDER], "Target %": [target_alloc[seg] for seg in ORDER]})
    ...
    fig_final = px.bar(final_melted, y="Segment", x="Value", color="Type", barmode="group", text="Value", orientation="h", color_discrete_map={"Current %": "#FFB703", "Target %": "#00B4D8"}, category_orders={"Segment": ORDER})
    fig_final.update_traces(texttemplate="%{x}%", textposition="outside", textfont=dict(size=14), cliponaxis=False)
    fig_final.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color="white", xaxis_title="Allocation (%)", xaxis=dict(gridcolor="#1f2630", range=[0, max_val * 1.3]), bargap=0.2, bargroupgap=0.1, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), margin=dict(t=40, b=10, l=80, r=80), height=350)
